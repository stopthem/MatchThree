...
public class Board : MonoBehaviour
{
    ...
    {
        int randomIndx = Random.Range(0, objectArray.Length);
        return objectArray[randomIndx];
    }
    ...
    private IEnumerator ClearAndRefillBoardRoutine(List<GamePiece> gamePieces)
    {
        ...
        if (m_boardDeadlock.IsDeadLocked(gamePiecesArray, 3))
        {
    ...
    private GameObject DropBomb(int x, int y, Vector2 swapDirection, List<GamePiece> gamePieces)
    {
        ...
        if (gamePieces.Count >= 4 && matchValue != MatchValue.None)
        {
            ...
            else
            {
                ...
                else if (swapDirection.x != 0 && matchValue != MatchValue.None)
                {
    ...
    private List<GamePiece> FindCollectiblesAt(int row, bool clearedAtBottomOnly = false)
    {
        ...
        for (int i = 0; i < width; i++)
        {
            if (gamePiecesArray[i, row] != null)
            {
                Collectible collectibleComponent = gamePiecesArray[i, row].GetComponent<Collectible>();

                if (collectibleComponent != null)
                {
                    if (!clearedAtBottomOnly || (clearedAtBottomOnly && collectibleComponent.clearedAtBottom))
                    {
                        foundCollectibles.Add(gamePiecesArray[i, row]);
                    }
    ...
    {
        return (Random.Range(0f, 1f) <= chanceForCollectibe && collectiblePrefabs.Length > 0 && collectibleCount < maxCollectibles);
    }
    ...
    {
        m_boardDeadlock.IsDeadLocked(gamePiecesArray, 3);
    }
    ...
    private IEnumerator ShuffleBoardRoutine()
    {
        ...
        List<GamePiece> normalPieces = m_boardShuffler.RemoveNormalPieces(gamePiecesArray);

        m_boardShuffler.ShuffleList(normalPieces);

        FillBoardFromList(normalPieces);

        m_boardShuffler.MovePieces(gamePiecesArray, .5f);
